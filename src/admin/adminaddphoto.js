...
const ImageUpload = () => {
  ...
  useEffect(() => {
    // Fetch uploaded photos from the backend and update the state
    axios
  ...
  const handleConfirmDelete = () => {
    axios
      .delete(`http://localhost:8000/gallery/${deleteConfirmationId}`)
      .then((response) => {
  ...
  return (
    <div style={{marginLeft:'50px',marginTop:'1.4rem'}}>
      ...
      <strong style={{ fontSize: "1.75rem" }}>Upload Images To Gallery</strong>
        ...
        <div className="uploaded-photos">
          {uploadedPhotos.map((photo) => (
            <div key={photo._id} className="">
              <img
                src={`http://localhost:8000/uploads/${photo.path}`}
                alt={photo.title}
                style={{height:'97px'}}
              />
              <br></br>
